...
SELECT count() FROM tab_lazy_rule WHERE hasAllTokens(s, ['bmid', 'cmid'])
    SETTINGS log_comment = '05217_rule_mid_pair_default';
...
SELECT count() FROM tab_lazy_rule WHERE hasAllTokens(s, ['adense', 'dsparse'])
    SETTINGS log_comment = '05217_rule_sparse_dense_default';

-- 'leapfrog' forces leapfrog even where the rule would pick brute force.
SELECT count() FROM tab_lazy_rule WHERE hasAllTokens(s, ['bmid', 'cmid'])
    SETTINGS text_index_postings_cursor_intersection_algorithm = 'leapfrog', log_comment = '05217_rule_mid_pair_leapfrog';

-- 'bruteforce' forces brute force even where the rule would pick leapfrog.
SELECT count() FROM tab_lazy_rule WHERE hasAllTokens(s, ['adense', 'dsparse'])
    SETTINGS text_index_postings_cursor_intersection_algorithm = 'bruteforce', log_comment = '05217_rule_sparse_dense_bruteforce';
...
(
    ...
    WHERE event_date >= yesterday() AND event_time >= now() - 600
      ...
      AND log_comment LIKE '05217_rule_%'
)
SELECT
