...
SELECT count() FROM tab_lazy_rule WHERE hasAllTokens(s, ['bmid', 'cmid'])
    SETTINGS text_index_lazy_intersection_density_threshold = 1.0, log_comment = '05217_rule_mid_pair_threshold_1';

-- Threshold 0 forces brute force even where the rule would pick leapfrog.
SELECT count() FROM tab_lazy_rule WHERE hasAllTokens(s, ['adense', 'dsparse'])
    SETTINGS text_index_lazy_intersection_density_threshold = 0.0, log_comment = '05217_rule_sparse_dense_threshold_0';
