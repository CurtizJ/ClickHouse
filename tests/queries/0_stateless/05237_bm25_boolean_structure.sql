...
SELECT count() > 0 FROM
(
    EXPLAIN SELECT id, bm25(1.5, 0.5) FROM tab_bm25_bool WHERE (hasToken(body, 'raft') AND price > 10) OR hasToken(body, 'stream')
)
WHERE explain LIKE '%BM25 scoring: index idx_body, scoring predicates: 2, k1: 1.5, b: 0.5%';
