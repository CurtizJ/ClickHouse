...
-- tasks, whose contexts are rebuilt from the initiator's user-level settings. The setting is
-- enabled explicitly here on purpose: the override is unconditional, because a worker cannot
-- honor it regardless of how it was set. use_skip_indexes_on_data_read, in contrast, is
-- supported: a worker re-derives all skip-index state from its own snapshot, so the initiator's
-- value must propagate to the worker tasks unchanged.
...
SELECT 'worker tasks: skip-index read propagates, JIT is disabled';
