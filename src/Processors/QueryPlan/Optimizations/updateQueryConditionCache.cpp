...
void updateQueryConditionCache(const Stack & stack, const QueryPlanOptimizationSettings & optimization_settings)
{
    ...
    for (auto iter = stack.rbegin() + 1; iter != stack.rend(); ++iter)
    {
        if (auto * filter_step = typeid_cast<FilterStep *>(iter->node->step.get()))
        {
            ...
            if (!filter_node || !VirtualColumnUtils::isDeterministic(filter_node))
                return;
            ...
            /// `ORDER BY ... LIMIT N` may drop granules during reading (the top-K filter is the first
            /// PREWHERE read step), so the result of the WHERE filter is no longer "applies to every
            /// granule of every part" — it applies only to the granules that the TopK filter decided
            /// to keep. To keep the QCC entry sound, we fold the deterministic part of the TopK plan
            /// into the cache key. Same query + same part set + same TopK params → cache hit;
            /// different LIMIT or sort column → fresh entry, never reusing a row-set computed under
            /// different TopK conditions.
