...
static bool mergeTreeReadCanBeShipped(const ReadFromMergeTree & read)
{
    ...
    /// Top-K filtering shares a runtime `TopKThresholdTracker` between the sorting step and the read, which
    /// applies `__topKFilter` as its first PREWHERE read step and skips granules by the threshold. The tracker
    /// cannot be shipped to a replica, so a replica would read without the threshold. Keeping such a read
    /// local is what `tryOptimizeTopK` does for `make_distributed_plan`, except that the decision has to be
    /// made here: enabling plan-based parallel replicas does not by itself mean the plan gets shipped, so
    /// suppressing Top-K up front would lose it for queries that stay local.
