...
namespace
{
    ...
    /// a table it knows: `PFor('double_delta')` from `MIN_WITH_PFOR_TIMESTAMP_CODEC` on, `DoubleDelta, ZSTD(1)`
    /// before it (see TimeSeriesVersion.h). All types accepted for the column (DateTime64/DateTime/UInt32)
    /// are compatible with both.
    ASTPtr getGeneratedTimestampCodec(UInt64 version)
    {
        if (version >= TimeSeriesVersion::MIN_WITH_PFOR_TIMESTAMP_CODEC)
            return makeASTFunction("CODEC", makeASTFunction("PFor", make_intrusive<ASTLiteral>(String{"double_delta"})));

        return makeASTFunction(
            "CODEC", make_intrusive<ASTIdentifier>("DoubleDelta"), makeASTFunction("ZSTD", make_intrusive<ASTLiteral>(UInt64{1})));
    }

    /// The compression codec of the auto-created `value` column of the samples tables: plain ZSTD(3), because
    /// specialized floating-point codecs such as Gorilla proved unreliable in practice.
    ASTPtr getGeneratedValueCodec()
    {
        return makeASTFunction("CODEC", makeASTFunction("ZSTD", make_intrusive<ASTLiteral>(UInt64{3})));
    }
            ...
            case ViewTarget::RecentSamples:
            {
                ...
                if (name == TimeSeriesColumnNames::Timestamp)
                {
                    if (!is_timestamp_type(*type))
                        return false;
                    auto generated_codec = getGeneratedTimestampCodec(settings[TimeSeriesSetting::version]);
                    return !codec || is_version_0
                        || (codec->formatWithSecretsOneLine() == generated_codec->formatWithSecretsOneLine());
                }
                ...
                if (name == TimeSeriesColumnNames::Value)
                {
                    if (!is_scalar_type(*type))
                        return false;
                    return !codec || is_version_0
                        || (codec->formatWithSecretsOneLine() == getGeneratedValueCodec()->formatWithSecretsOneLine());
                }
