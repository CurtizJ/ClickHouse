...
/// of default values (`num_defaults`, sampled via `getRatioOfDefaultRows`). This is what lets the
/// writer derive serialization kinds (sparse vs dense) from statistics instead of storing the
/// default count separately in `serialization.json` (see `loadSerializationInfosFromStatistics`).
