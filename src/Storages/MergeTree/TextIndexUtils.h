...
private:
    ...
    /// Returns std::nullopt if the row was dropped by the merge.
    std::optional<UInt32> adjustPartOffset(size_t part_index, UInt32 row_id) const;

    /// Unions the given row ids into output_postings_bitmap.
    void appendPostingsToBitmap(std::span<UInt32> row_ids);
    /// Adjusts the row ids of one source and appends those that survived the merge
    /// to output_postings_array or output_postings_bitmap.
