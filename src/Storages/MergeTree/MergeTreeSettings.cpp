...
#define MERGE_TREE_SETTINGS(DECLARE, DECLARE_WITH_ALIAS) \
    ...
    **See Also**
    ...
    - `without_data` - Like `with_types`, but the per-column number of rows and default values is no longer stored
    in `serialization.json`. Instead, serialization kinds (e.g. sparse) are derived from column statistics at write time.
    ...
    switch to `with_types` or `without_data` to enable per-type serialization versions.
    )", 0) \
