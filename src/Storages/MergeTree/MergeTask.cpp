...
bool MergeTask::ExecuteAndFinalizeHorizontalPart::prepare() const
{
    ...
    /// Choose the serialization of the merged part from the serialization of the source parts.
    /// The per-column default counts that this decision used to be based on are no longer stored in
    /// the serialization info (they are derived from statistics at write time), so we carry over the
    /// serialization kinds that the source parts already chose: a column stays sparse only if every
    /// source part that contains it serialized it with a custom (e.g. sparse) serialization. A column
    /// absent from a part contributes only default values, so it does not prevent sparse serialization;
    /// a column serialized as plain Default in any source part forces the merged part to use the
    /// default serialization too.
    std::unordered_map<String, MutableSerializationInfoPtr> custom_infos_from_parts;
    std::unordered_set<String> force_default_columns;
    ...
    for (const auto & part : global_ctx->future_part->parts)
    {
        if (!info_settings.isAlwaysDefault())
        {
            ...
            {
                auto it = part_infos.find(name);
                if (it == part_infos.end())
                    continue;

                if (it->second->hasCustomSerialization())
                    custom_infos_from_parts.emplace(name, it->second->clone());
                else
                    force_default_columns.insert(name);
            }
        }
