...
std::optional<UInt32> MergeTextIndexesTask::adjustPartOffset(size_t part_index, UInt32 row_id) const
{
    chassert(merged_part_offsets);
    auto new_offset = merged_part_offsets->tryGetNewOffset(part_index, row_id);
...
void MergeTextIndexesTask::appendPostings(size_t source_num, std::span<UInt32> row_ids)
{
    ...
    if (merged_part_offsets)
    {
        ...
        {
            /// Skip the rows dropped by the merge.
            if (auto new_row_id = adjustPartOffset(part_index, row_id))
                row_ids[num_remapped++] = *new_row_id;
        }

        row_ids = row_ids.first(num_remapped);
        if (row_ids.empty())
            return;
    }
...
void MergeTextIndexesTask::readAndAppendPositions(size_t source_num, TokenPostingsInfo & token_info)
{
    ...
    if (merged_part_offsets)
    {
        ...
        for (const auto & entry : position_entries_buffer)
        {
            /// Skip the rows dropped by the merge.
            if (auto new_doc_id = adjustPartOffset(part_index, entry.doc_id))
                position_entries_buffer[num_remapped++] = entry.withDocId(*new_doc_id);
        }

        position_entries_buffer.resize(num_remapped);
    }
    ...
    do
    {
        ...
        for (size_t i = 0; i < batch_size; ++i, ++row)
        {
            if (i > 0 || first_row_is_new_token)
            {
                ...
                /// in output_postings_array or output_postings_bitmap and must be flushed
                /// (or the token dropped if they are empty).
